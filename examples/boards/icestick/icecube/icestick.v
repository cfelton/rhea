...
reg [3:0] uart_inst_instrxfifo_nvacant;
reg uart_inst_instrxfifo_full;
reg [2:0] uart_inst_instrxfifo_addr;
wire uart_inst_instrxfifo_fbus_clear;
wire [3:0] uart_inst_instrxfifo_fbus_count;
reg [3:0] uart_inst_instrxfifo_ntenant;
reg [3:0] uart_inst_insttxfifo_nvacant;
reg [2:0] uart_inst_insttxfifo_addr;
wire uart_inst_insttxfifo_fbus_rvld;
wire uart_inst_insttxfifo_fbus_clear;
wire [3:0] uart_inst_insttxfifo_fbus_count;
reg [3:0] uart_inst_insttxfifo_ntenant;
reg [13:0] tick_inst_mscnt;
reg [9:0] tick_inst_seccnt;
reg tick_inst_g2_increment;

reg [7:0] cmd_inst_packet [0:20-1];
reg [7:0] uart_inst_instrxfifo_mem [0:8-1];
reg [7:0] uart_inst_insttxfifo_mem [0:8-1];
...
assign cmd_inst_address[32-1:24] = cmd_inst_packet[4];
assign cmd_inst_address[24-1:16] = cmd_inst_packet[5];
assign cmd_inst_address[16-1:8] = cmd_inst_packet[6];
assign cmd_inst_address[8-1:0] = cmd_inst_packet[7];
assign cmd_inst_data[32-1:24] = cmd_inst_packet[16];
assign cmd_inst_data[24-1:16] = cmd_inst_packet[17];
assign cmd_inst_data[16-1:8] = cmd_inst_packet[18];
assign cmd_inst_data[8-1:0] = cmd_inst_packet[19];
...
always @(posedge clock) begin: ICESTICK_UART_INST_INSTTXFIFO_GENS_3
    if (cmd_inst_reset == 1) begin
        ...
        if (uart_inst_insttxfifo_fbus_clear) begin
            ...
            if (($signed({1'b0, uart_inst_insttxfifo_addr}) == (8 - 2))) begin
...
always @(posedge clock) begin: ICESTICK_UART_INST_INSTTXFIFO_GENS_4
    if (cmd_inst_reset == 1) begin
        uart_inst_insttxfifo_nvacant <= 8;
...
always @(posedge clock) begin: ICESTICK_UART_INST_INSTRXFIFO_GENS_3
    if (cmd_inst_reset == 1) begin
        ...
        if (uart_inst_instrxfifo_fbus_clear) begin
            ...
            if (($signed({1'b0, uart_inst_instrxfifo_addr}) == (8 - 2))) begin
...
always @(posedge clock) begin: ICESTICK_UART_INST_INSTRXFIFO_GENS_4
    if (cmd_inst_reset == 1) begin
        uart_inst_instrxfifo_nvacant <= 8;
...
always @(posedge clock) begin: ICESTICK_CMD_INST_BEH_STATE_MACHINE
    ...
    if (cmd_inst_reset == 1) begin
        ...
        cmd_inst_packet[12] <= 0;
        cmd_inst_packet[13] <= 0;
        cmd_inst_packet[14] <= 0;
        cmd_inst_packet[15] <= 0;
        cmd_inst_packet[16] <= 0;
        cmd_inst_packet[17] <= 0;
        cmd_inst_packet[18] <= 0;
        cmd_inst_packet[19] <= 0;
        cmd_inst_fbtx_wr <= 0;
        ...
        case (cmd_inst_state)
            ...
            4'b0110: begin
                memmap_read <= 1'b0;
                if (memmap_done) begin
                    cmd_inst_packet[16] <= memmap_read_data[32-1:24];
                    cmd_inst_packet[17] <= memmap_read_data[24-1:16];
                    cmd_inst_packet[18] <= memmap_read_data[16-1:8];
                    cmd_inst_packet[19] <= memmap_read_data[8-1:0];
